...
func (o *Instance) Name() string {
	return "ext_unrealized"
}

func (o *Instance) Description() string {
	desc_eng := `Extended unrealized justification reorg attack`
	return desc_eng
}
	...
	for {
		...
		case <-ticker.C:
			...
			if hackDuties, happen := CheckDuties(params, duties); happen {
				...
				strategy.Slots = GenSlotStrategy(hackDuties, params.FilterHackerDuties(duties))
